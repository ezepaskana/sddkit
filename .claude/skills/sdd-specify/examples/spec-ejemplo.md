...
- **Metrica:** pasos manuales necesarios para actualizar sddkit en un
  repo existente.
- **Baseline actual:** tras `npm update sddkit`, el dev debe correr
  `sdd setup` (wizard completo) o copiar skills a mano y editar config.
  ~5-8 pasos manuales, propenso a olvidos (skills desactualizadas,
  hooks sin reinstalar).
- **Resultado esperado:** un solo comando (`sdd sync`) sin interaccion.
  De un proceso manual propenso a errores a un comando idempotente.
- **Como se mide despues:** (1) `sdd sync` exit 0 con resumen correcto
  en los 3 escenarios; (2) `sdd doctor` sin warnings tras `sync`;
  (3) diff confirma que `installSkills` deja la carpeta identica al
  paquete (sin archivos huerfanos).

_Metrica cualitativa: el valor es reducir friccion de actualizacion y
evitar desincronizacion silenciosa de skills/hooks entre versiones._
