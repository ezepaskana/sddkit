...
- **BR-073** — "SI el repo es un monorepo (BR-069 detectó 2+ módulos), el agente genera además un `CLAUDE.md` en la raíz de cada módulo con su responsabilidad, las capas que contiene, su relación con los otros módulos y **sus símbolos de entrada**: lo que ese módulo expone y quién lo consume (clases públicas, funciones exportadas, interfaces compartidas — nunca los internos). En un repo de módulo único NO se genera ningún `CLAUDE.md` anidado — el bloque gestionado de la raíz ya cubre ese caso". Fuente: tarea 019. _(ampliada en tarea 022 con los símbolos de entrada, para que el agente sepa qué toca sin abrir el paquete entero)._
...
- **BR-088** — "**La documentación de arquitectura se escribe para el agente, no para un lector humano**: `.sdd/c4/` y los `CLAUDE.md` de módulo usan tablas con rutas de archivo y símbolos reales, no prosa descriptiva. No documentan lo que el agente puede deducir del archivo que ya tiene abierto: el doc **ubica y conecta**, no explica el código". Fuente: tarea 022.
- **BR-089** — "**Índice arriba, detalle abajo.** SI el repo tiene 2+ módulos, `.sdd/c4/components.md` es un índice —una fila por módulo con su ruta, su responsabilidad en una línea y sus dependencias— y el detalle vive en el `CLAUDE.md` de cada módulo, que carga solo al tocarlo (BR-073, ADR-0015). SI el repo tiene un solo módulo, el detalle se queda en `components.md`. Los tres niveles de `.sdd/c4/` respetan el tope de 45 líneas (BR-082): si un nivel no entra, se parte por módulo, no se lo deja crecer". Fuente: tarea 022.
- **BR-090** — "**La capa dice CÓMO se escribe el código; el módulo dice QUÉ hace y de quién depende.** Ninguno de los dos repite lo del otro: una convención de escritura va en `.claude/rules/sdd-layer-<capa>.md` y jamás en el `CLAUDE.md` de un módulo ni en el bloque de la raíz; la responsabilidad y las dependencias de un módulo van en su `CLAUDE.md` y jamás en una rule de capa". Fuente: tarea 022.
...
- **BR-091** — "El plugin registra un tercer hook `SessionStart` que vuelca `hooks/caveman.md` en TODA sesión, salvo que `.sdd/config.json` declare `ui.caveman` en `'no'` — el modo caveman está **activo por default** para cualquier repo que instale sddkit, y apagarlo es un acto explícito del dev. Caveman aplica ÚNICAMENTE a lo que el agente le dice al dev en el chat: los artefactos SDD, los docs de `.sdd/`, el código, los mensajes de commit y los PR se escriben siempre en prosa normal. Las reglas de brevedad (BR-064, BR-066, BR-067, BR-068) mandan por encima: caveman comprime la redacción de esa estructura, no la reemplaza. El dev también lo apaga por turno diciendo `normal` o `basta caveman`, sin que eso toque ningún archivo". Fuente: tarea 023.
