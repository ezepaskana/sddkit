...
4. Flujo para tareas no triviales (toda tarea con más de un archivo o cualquier ambigüedad): \`sdd task new "<requisito verbatim>"\` → completá analysis.md (análisis crítico, preguntas, métrica de impacto) → APROBACIÓN del dev → \`sdd task status <id> analyzed\` → completá spec.md (criterios EARS, reglas de negocio, fuera de alcance) → APROBACIÓN del dev → completá plan.md con pasos chicos verificables → APROBACIÓN → \`sdd task status <id> in-progress\` y ejecutá un paso a la vez marcando checkboxes en plan.md. Pausar: \`sdd task status <id> paused\`; retomar: \`sdd task show <id>\`. Cerrar: \`sdd task status <id> done\` (actualizando .sdd/c4/ si cambió la arquitectura).
